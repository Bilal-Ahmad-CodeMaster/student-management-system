...
import { Edit3, Trash2, Plus, Search, UserCheck, BookOpen } from "lucide-react";
import CourseModal from "@/app/components/modals/CourseModal";
import ConfirmModal from "@/app/components/modals/ConfirmModal";
import AssignModal from "@/app/components/modals/AssignModal"; // Import the modal
...
export default function CourseManagement() {
  ...
  const [isAssignOpen, setIsAssignOpen] = useState(false); // Assign Modal State
    ...
    <div className="space-y-6 text-slate-900">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Organize curriculum and manage enrollments.</p>
        </div>
        <button
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="p-4 text-xs font-bold text-slate-500 uppercase">Course Details</th>
              <th className="p-4 text-xs font-bold text-slate-500 uppercase">Instructors</th>
              <th className="p-4 text-xs font-bold text-slate-500 uppercase">Enrolled</th>
              <th className="p-4 text-xs font-bold text-slate-500 uppercase text-center">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan="4" className="p-10 text-center text-slate-400 font-medium tracking-tight">Loading Courses...</td></tr>
              ...
              <tr key={c._id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-4">
                  <p className="text-xs font-medium text-slate-600 truncate max-w-[150px]">
                    {c.instructors.length > 0 ? c.instructors.map(i => i.name).join(", ") : "Not Assigned"}
                  </p>
                </td>
                <td className="p-4">
                  <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded-lg text-[10px] font-bold">
                    {c.assignedUsersCount} Students
                  </span>
                </td>
                <td className="p-4">
                  <div className="flex justify-center items-center gap-1">
                    <button onClick={() => { setCurrentCourse(c); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-slate-900 transition-all rounded-lg" title="Edit">
                      <Edit3 size={18} />
                    </button>
                    <button onClick={() => openAssignModal(c)} className="p-2 text-slate-400 hover:text-blue-600 transition-all rounded-lg" title="Assign Students/Teachers">
                      <UserCheck size={18} />
                    </button>
                    <button onClick={() => openDeleteConfirm(c._id)} className="p-2 text-slate-400 hover:text-red-600 transition-all rounded-lg" title="Delete">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </td>
      ...
      <ConfirmModal
        ...
        title="Remove Course"
        message="Are you sure? This will remove the course and unassign all students."
      />
    </div>
  );
}